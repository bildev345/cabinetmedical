...
@section('main')
    <div class="mx-auto p-4">
        <h1 class="text-4xl font-bold mb-4 text-green-900">Modifier le document '{{ basename($document->fichier) }}'</h1>
        <form action="{{ route('documents.update', $document) }}" method="POST" enctype="multipart/form-data" class="bg-green-50 p-6 rounded-lg shadow-md">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="date" class="block mb-1 text-green-800">Date:</label>
                <input type="date" name="date" id="date" class="border border-green-300 p-2 w-full rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('date') border-red-500 @enderror" value="{{ old('date', $document->date) }}">
                @error('date')
            ...
            <div class="mb-4">
                ...
                <textarea name="rapport" id="rapport" class="border border-green-300 p-2 w-full rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('rapport') border-red-500 @enderror">{{ old('rapport', $document->rapport) }}</textarea>
                @error('rapport')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                <label for="fichier" class="block mb-1 text-green-800">Document (optional, any type):</label>
                ...
                @enderror
                @if ($document->fichier)
                    <p class="text-green-700 mt-2">Current file: {{ basename($document->fichier) }}</p>
                @endif
            </div>
            <div class="mb-4">
                ...
                <select name="type_document_id" id="type_document_id" class="border border-green-300 p-2 w-full rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('type_document_id') border-red-500 @enderror">
                    @foreach ($typeDocuments as $type)
                        <option value="{{ $type->id }}" {{ old('type_document_id', $document->type_document_id) == $type->id ? 'selected' : '' }}>{{ $type->type_document }}</option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select name="patient_id" id="patient_id" class="border border-green-300 p-2 w-full rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 @error('patient_id') border-red-500 @enderror">
                    @foreach ($patients as $patient)
                        <option value="{{ $patient->id }}" {{ old('patient_id', $document->patient_id) == $patient->id ? 'selected' : '' }}>{{ $patient->nom }} {{ $patient->prenom }}</option>
                    @endforeach
                </select>
                @error('patient_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
